...
commonbook reads and writes **directly** against articles.astrojason.com's shared Turso/libSQL DB (`api/_lib/turso.ts`) — no HTTP call into that app's Go backend. The schema mirrors the queries in `articles.astrojason.com/backend/internal/handlers/{library,read,categories}.go`: `article` (`id, title, url, summary, reading_time, keywords` as a JSON array string, `content, created_by_uid, created_timestamp`), `user_article_read` (`article_id, created_by_uid, ...`), `user_article_category` (`id, name, created_by_uid`), `user_article_category_article` (join table). `article.keywords` is a controlled-taxonomy field (`keyword_taxonomy.md` in that repo) distinct from the older free-text `search_terms` column — commonbook matches against `keywords`, not `search_terms`.
...
`GET /api/discover/feed` — verifies the caller's Firebase ID token has app access (USER/ADMIN/SUPERADMIN), then returns unread articles for `articlesAppUid` matching at least one of the caller-supplied `keywords` against the article's `keywords` column (empty keyword list ⇒ empty result, by design — Discover never falls back to "all unread"), optionally narrowed by free-text `search` or a `category`. Also returns the full category list for that uid.
